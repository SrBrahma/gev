...
export async function addPackages({
  ...
  installPackages,
  packageManager,
}: AddPackagesProps): Promise<void> {
  await ensurePackageManagerIsSetup({ packageManager, projectPath });
  ...
  if (installPackages)
    await oraPromise(async () => {
      ...
      else if (packageManager === 'yarn') await execa('yarn', ['install'], { cwd: projectPath });
      else await execa('pnpm', ['install', '--ignore-scripts'], { cwd: projectPath });
